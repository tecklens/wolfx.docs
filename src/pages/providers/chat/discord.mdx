...
Right now, Wolfx only supports the **Discord Webhook** integration. This approach has been because the easiest way to set it up is when you have a predefined destination for where the notifications should be sent. It's particularly useful for notifying a specific channel about updates.

**Quickstart:**

To get started with using Wolfx’s Discord Webhook integration, you need a ‘webhook token’. Here’s how you can generate the same for testing purposes:
...
5. After obtaining the webhook URL in the previous step, you need to store it within the subscriber entity. Doing this ensures that Wolfx knows where (in which discord channel) to send the notification to. Here’s how to do it:
