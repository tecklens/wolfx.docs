...
```jsx
import { Wolfx, ChatProviderIdEnum } from '@novu/node';

const novu = new Wolfx("<WOLFX_API_KEY>");

await novu.subscribers.setCredentials('subscriberId', ChatProviderIdEnum.Mattermost, { webhookUrl: '<enter-mattermost-webhook-url>', });
...
await novu.subscribers.setCredentials('subscriberId', ChatProviderIdEnum.Mattermost, { botApiToken: '<enter-mattermost-bot-api-token>' });
