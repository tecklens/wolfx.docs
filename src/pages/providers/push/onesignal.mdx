...
In order to target the OneSignal user from Wolfx, you must register the OneSignal `player_id`as the `deviceToken` for your Wolfx subscriber. This value can be retrieved via the [OneSignal SDK](https://documentation.onesignal.com/docs/users-and-devices#finding-users) for your platform.

Once you have the user's `player_id` value, the `deviceToken` for your Wolfx subscriber can be set via:
